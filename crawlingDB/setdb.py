    ...
    for i in range(1, y+1):  # 1,2 페이지 크롤링
        ...
        for tweet in tweets:
             c.execute("INSERT INTO twitterdb (time, content, likey, retweet, keyword, feeling) \
                VALUES(?,?,?,?,?,?)", (tweet.created_at, tweet.text, tweet.favorite_count, tweet.retweet_count, keyword, feeling))
...
def naverBlogOpenAPI(x,y,z,w):
    ...
    for i in range(1, y+1): 
        if i == 1:
            url = "https://openapi.naver.com/v1/search/blog?query=" + keyword + "&display=" + str(z) + "&start=" + str(i)
        elif i > 1 :
            url = "https://openapi.naver.com/v1/search/blog?query=" + keyword + "&display=" + str(z) + "&start=" + str((i-1)*z + 1)
        ...
        for item in json_data['items']:
            ...
            c.execute("INSERT INTO naver (content, title, link, postdate, keyword, feeling) VALUES(?,?,?,?,?,?)", (de, ti, li, po, keyword, feeling))
